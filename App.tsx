    ...
    const loadData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Merge static data with dynamic JSON to ensure all required fields exist
      ...
      <header className="fixed top-0 left-0 w-full z-50 p-6 border-b border-white/5 bg-black/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
             <div className="hidden lg:flex items-center gap-2 text-[10px] text-zinc-500 font-bold uppercase">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-40 pb-20">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-24 items-center">
          <div className="lg:col-span-7">
            <div className="inline-block px-3 py-1 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-[10px] font-bold uppercase tracking-[0.3em] mb-6">
              Project_Bio_V2.0
            </div>
            <h1 className="text-5xl md:text-7xl font-black text-white leading-tight uppercase tracking-tighter mb-8">
              CYBER <span className="text-cyan-400">INFRASTRUCTURE</span> SPECIALIST
            </h1>
            <p className="text-lg text-slate-400 max-w-xl leading-relaxed mb-10 border-l-2 border-cyan-500/30 pl-8">
              Expertise in <span className="text-white font-bold">Digital Forensics</span>, 
              <span className="text-white font-bold"> Cybersecurity</span>, and enterprise-scale 
              IT infrastructure management.
            </p>
            <div className="flex gap-8">
               <div>
                 <div className="text-[9px] text-slate-600 font-bold uppercase tracking-widest mb-1">Identity</div>
                 <div className="text-xs font-bold text-white uppercase">{data.name}</div>
               </div>
               <div>
                 <div className="text-[9px] text-slate-600 font-bold uppercase tracking-widest mb-1">Role</div>
                 <div className="text-xs font-bold text-cyan-500 uppercase">{data.title}</div>
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative">
            <div className="p-3 border border-white/10 bg-white/5 backdrop-blur-md rounded-2xl overflow-hidden group">
               ...
               />
               <div className="absolute top-6 left-6 text-[8px] font-bold bg-black/80 px-2 py-1 border border-cyan-500/20 text-cyan-400 uppercase">
                 ID_SCAN: {data.name.split(' ')[0]}
               </div>
            </div>
          </div>
        </section>
